...
// Imagens importadas como MÓDULO, não por caminho "/arquivo.png".
// O Vite resolve, empacota e versiona — não depende da pasta public/
// existir no deploy, que era a causa das imagens sumirem.
import logoYnove from "./logo-ynove.png";
import mascoteYnove from "./mascote-ynove-busto.jpg";
import {
  ChevronLeft,
  MessageCircle,
  RotateCcw,
...
function temSinalTecnico(familiaId, answers) {
  const temGrao = graoAlvo(familiaId, answers) != null;
  // Para escova, a bitola da esmerilhadeira NÃO é sinal técnico: o diâmetro
  // cadastrado é o da própria escova, e a montagem é por rosca M14 (serve em
  // qualquer máquina). Contar essa bitola como sinal faria a tela prometer
  // uma "recomendação" que o motor não tem base para fazer.
  const temDiametro = familiaId !== "escova" && diametroProvavel(answers) != null;
  return temGrao || temDiametro;
}

function buscarProdutos(familiaId, answers) {
  ...
  const temGrao = alvoGrao != null;
  const temDiametroUtil = familiaId !== "escova" && alvoDiam != null;
  const semSinal = !temGrao && !temDiametroUtil;
  ...
  // Devolve TODOS os compatíveis. Nenhum SKU tecnicamente adequado é
  // escondido por limite de exibição — a interface decide como apresentar
  // (primeiro em destaque, demais como "outras opções").
  return pontuado.map((x) => x.p);
}
  ...
  if (familiaIds.length === 1) {
    ...
    if (answers.operacao === "Remover solda" && precisaRefinoSolda(answers.depoisDisso)) {
      ...
      return {
        status: "duas-etapas",
        principal: familia,
        // ETAPA 1 — produtos da família de remoção.
        produtosPrincipal: buscarProdutos(familia.id, answers),
        // ETAPA 2 — cada família de refino já vem com seus próprios produtos,
        // num objeto só. Evita dois arrays paralelos saindo de sincronia.
        // refinoIds vazio => refinos vazio => a tela mostra a mensagem de
        // especialista, sem inventar produto.
        refinos: refinoIds.map((id) => ({
          familia: FAMILIAS[id],
          produtos: buscarProdutos(id, answers),
        })),
        confidence,
      };
    }
...
function buildWhatsAppMessage(answers, result) {
  ...
  if (result?.status === "direct" || result?.status === "familia-sem-produto-validado" || result?.status === "sequence") {
    ...
    const p1 = result.produtosPrincipal?.[0];
    if (p1) linhas.push(`  Produto sugerido: ${p1.nome} — ${money(p1.preco)}`);

    const refs = result.refinos || [];
    if (refs.length > 0) {
      linhas.push(`Etapa 2 (refino): ${refs.map((r) => r.familia.nome).join(" ou ")}`);
      refs.forEach((r) => {
        const p2 = r.produtos?.[0];
        if (p2) linhas.push(`  ${r.familia.nome}: ${p2.nome} — ${money(p2.preco)}`);
      });
    } else {
...
// Paleta clara da identidade YNOVE.
// "white" mantém o nome por compatibilidade com o resto do arquivo, mas
// agora é a cor do TEXTO PRINCIPAL (grafite) sobre fundo claro.
const TOKENS = {
  bg: "#F5F8FC",      // fundo principal
  panel: "#FFFFFF",   // cartões e superfícies
  steel: "#E3EAF2",   // bordas
  blue: "#1769D2",    // azul institucional
  teal: "#19B8B5",    // turquesa
  white: "#172B4D",   // texto principal (grafite)
  mute: "#5A6B80",    // texto secundário
  warn: "#C2410C",    // alertas (laranja-queimado, sem amarelo)
};
    ...
    <div style={styles.page}>
      ...
      <div style={styles.shell}>
        ...
        {view === "result" && result?.status === "duas-etapas" && (
          <DuasEtapasResult answers={answers} result={result} onWhatsApp={handleWhatsAppClick} onRestart={restart} onComprar={handleComprarClick} />
        )}
...
// Mostra os primeiros itens e permite abrir o restante. NENHUM produto
// compatível é descartado — só ficam recolhidos para a tela não virar uma
// lista de 40 cards de uma vez.
...
function BlocoProdutos({ produtos, familia, answers, onComprar, modoLista }) {
  ...
  const inicio = modoLista ? 0 : 1;
  const principal = modoLista ? null : produtos[0];
  const restantes = produtos.slice(inicio);
  const mostrados = expandido ? restantes : restantes.slice(0, VISIVEIS_INICIAL);
  const ocultos = restantes.length - mostrados.length;
    ...
    <>
      {modoLista ? (
        ...
        <>
          ...
          {restantes.length > 0 && (
            <div style={styles.subtituloAlt}>
              OUTRAS OPÇÕES COMPATÍVEIS ({restantes.length})
            </div>
          )}
        </>
      )}
      ...
      {mostrados.map((p) => (
        <ProdutoCard key={p.id} p={p} familia={familia} answers={answers}
                     destaque={false} onComprar={onComprar} modoLista={modoLista} />
      ))}

      {ocultos > 0 && (
        <button className="yn-btn" style={styles.verMais} onClick={() => setExpandido(true)}>
          Ver mais {ocultos} {ocultos === 1 ? "opção compatível" : "opções compatíveis"}
        </button>
      )}
    </>
  );
...
function DuasEtapasResult({ answers, result, onWhatsApp, onRestart, onComprar }) {
  const refinos = result.refinos || [];
  const temRefinoDefinido = refinos.length > 0;
  ...
  return (
    <div className="yn-fadein" style={styles.resultWrap}>
      ...
      <div style={{ marginTop: 12, marginBottom: 26 }}>
        <BlocoProdutos
          produtos={result.produtosPrincipal || []}
          familia={result.principal}
          answers={answers}
          onComprar={onComprar}
          modoLista={!temSinalTecnico(result.principal.id, answers)}
        />
      </div>
      ...
      <div style={styles.resultCard}>
        ...
        {temRefinoDefinido ? (
          refinos.map((r) => (
            <div key={r.familia.id} style={{ marginBottom: 6 }}>
              <span style={styles.categoriaTag}>{r.familia.nome}</span>
              <p style={styles.porqueText}>{r.familia.descricaoCurta}</p>
            </div>
          ))
        ) : (
          <div style={styles.atencaoBox}>
            <AlertTriangle size={16} color={TOKENS.warn} style={{ flexShrink: 0, marginTop: 2 }} />
            <span>
              O acabamento que você precisa exige uma segunda etapa, mas a família certa pra ela
              depende do nível de acabamento final — pode ser flap mais fino, abrasivo revestido ou
              não tecido. Um especialista da YNOVE define isso com você.
            </span>
          </div>
        )}
      </div>
      {refinos.map((r) => (
        <div key={r.familia.id} style={{ marginTop: 12, marginBottom: 20 }}>
          <BlocoProdutos
            produtos={r.produtos || []}
            familia={r.familia}
            answers={answers}
            onComprar={onComprar}
            modoLista={!temSinalTecnico(r.familia.id, answers)}
          />
        </div>
      ))}
      ...
      {result.familias.map((f) => {
        ...
        return (
          <div key={f.id} style={{ marginBottom: 26 }}>
            <div style={{ ...styles.resultCard, marginBottom: prods.length ? 12 : 0 }}>
            ...
            <BlocoProdutos produtos={prods} familia={f} answers={answers}
                           onComprar={onComprar} modoLista={!temSinalTecnico(f.id, answers)} />
          </div>
        );
      })}
      <InoxNote material={answers.material} />
...
// Mascote YNOVE (tatu soldador). Importado como módulo no topo do arquivo.

function Mascote({ size = 132 }) {
  const [erro, setErro] = useState(false);
  if (erro) return null;
  const arq = mascoteYnove;
  return (
    <img
      src={arq}
      alt="Mascote YNOVE"
      ...
      style={{
        ...
        borderRadius: "50%",
        border: `2px solid ${TOKENS.steel}`,
      }}
    />
  );
}

// Logo oficial YNOVE Soldas & Abrasivos. Importada como módulo — o Vite
// resolve o caminho e versiona o arquivo. Se ainda assim falhar, cai no
// texto "YNOVE" sem quebrar a tela.
function YnoveLogo({ height = 30 }) {
  const [erro, setErro] = useState(false);
  if (erro) {
    return (
      <span style={{ fontFamily: "'Oswald', sans-serif", fontWeight: 700, fontSize: 17,
                     letterSpacing: "0.02em", backgroundImage: GRAD,
  ...
  return (
    <img src={logoYnove} alt="YNOVE Soldas & Abrasivos" height={height}
...
const styles = {
  ...
  optionBtn: { display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", textAlign: "left", padding: "16px 18px", borderRadius: 10, background: TOKENS.panel, border: `1px solid ${TOKENS.steel}`, boxShadow: "0 1px 2px rgba(23,43,77,0.05)", color: TOKENS.white, fontFamily: "'Inter', sans-serif", fontSize: 14.5, fontWeight: 500, cursor: "pointer" },
  ...
  resultCard: { background: TOKENS.panel, border: `1px solid ${TOKENS.steel}`, borderRadius: 14, padding: "22px 20px", boxShadow: "0 1px 3px rgba(23,43,77,0.06)" },
  ...
  atencaoBox: { display: "flex", gap: 10, marginTop: 14, padding: "12px 14px", background: "#FFF4EF", border: `1px solid #F5C6B0`, borderRadius: 8, fontSize: 12.5, color: TOKENS.white, lineHeight: 1.5 },
  ...
  ctaComprar: { marginTop: 18, display: "flex", alignItems: "center", justifyContent: "center", gap: 9, padding: "17px 18px", borderRadius: 10, background: GRAD, color: "#FFFFFF", fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 15.5, letterSpacing: "0.02em", textDecoration: "none", cursor: "pointer", border: "none" },
  ...
  ctaPrimary: { marginTop: 22, display: "flex", alignItems: "center", justifyContent: "center", gap: 9, padding: "16px 18px", borderRadius: 10, background: GRAD, color: "#FFFFFF", fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 15.5, textDecoration: "none", cursor: "pointer", border: "none" },
